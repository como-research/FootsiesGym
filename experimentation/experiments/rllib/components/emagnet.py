...
from ray.rllib.utils.torch_utils import explained_variance, global_norm, sequence_mask
...
class EMAgnetTorchPolicy(APPOTorchPolicy):
    ...
    def loss(
        ...
        model.tower_stats["emag_loss"] = mmd_loss
        model.tower_stats["emag_kl_div"] = torch.mean(kl_div)
    ...
    def stats_fn(self, train_batch: SampleBatch) -> Dict[str, TensorType]:
        """Stats function for EMAgnetAPPO. Returns a dict with important loss stats.
        ...
        stats_dict["emag_loss"] = torch.mean(
            torch.stack(self.get_tower_stats("emag_loss"))
        )
        stats_dict["emag_kl_div"] = torch.mean(
            torch.stack(self.get_tower_stats("emag_kl_div"))
        )
