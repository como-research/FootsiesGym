    ...
    def step(
        ...
        for i in range(N):
            if self._autoreset_envs[i]:
                # Fill NONE for auto-resetting envs — FootsiesEnv
                # already auto-reset internally, so stepping with
                # NONE advances one benign frame.
                p1_actions[i] = constants.EnvActions.NONE
                p2_actions[i] = constants.EnvActions.NONE
            else:
                act = actions[i]
        ...
        for i in range(N):
            if self._autoreset_envs[i]:
                # Auto-reset env: return post-reset obs (from
                # FootsiesEnv's internal auto-reset), zero rewards,
                # not done.
                observations.append({a: obs_dict[a][i] for a in ("p1", "p2")})
                rewards.append({"p1": 0.0, "p2": 0.0})
                terminateds.append(
                    {"p1": False, "p2": False, "__all__": False}
                )
                truncateds.append({"p1": False, "p2": False, "__all__": False})
                infos.append({"p1": {}, "p2": {}})
                # Clear the flag — next step will be normal.
                self._autoreset_envs[i] = False
            else:
                obs_i = {a: obs_dict[a][i] for a in ("p1", "p2")}
                rew_i = {a: float(rew_dict[a][i]) for a in ("p1", "p2")}
                p1_term = bool(term_dict["p1"][i])
                p2_term = bool(term_dict["p2"][i])
                p1_trunc = bool(trunc_dict["p1"][i])
                p2_trunc = bool(trunc_dict["p2"][i])
                all_done = (p1_term or p2_term) or (p1_trunc or p2_trunc)
                ...
                # Mark for auto-reset on next step.
                if all_done:
                    self._autoreset_envs[i] = True
