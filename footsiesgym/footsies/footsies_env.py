...
class FootsiesEnv(ParallelEnv):
    ...
    def _launch_binaries(self, port: int):
        ...
        if system == "linux":
            ...
            raise RuntimeError(
                "Binary launching is only supported on Linux and "
                "macOS. Please launch the footsies server manually."
            )
        ...
        binary_path = os.path.join(
            ...
            "binaries",
            binary_subdir,
            binary_manager.executable_relpath(binary_platform, self.headless),
        )
        ...
        if binary_platform == "mac":
            # Ad-hoc re-sign so Gatekeeper allows the downloaded binary,
            # then run under Rosetta (Grpc.Core is x86_64-only).
            sign_target = (
                os.path.join(project_root, "binaries", binary_subdir)
