...
from footsiesgym.footsies.typing import ActionBits, ActionType, AgentID, EnvID
...
class FootsiesBot:
    ...
    def __init__(self, frame_skip: int = 4):
        ...
        self.move_queues: dict[EnvID, dict[AgentID, collections.deque[ActionType]]] = (
            collections.defaultdict(lambda: collections.defaultdict(collections.deque))
        )
        self.attack_queues: dict[
            EnvID, dict[AgentID, collections.deque[ActionType]]
        ] = collections.defaultdict(lambda: collections.defaultdict(collections.deque))
    ...
    def get_next_input(
        ...
        if (
            random.rand() < quick_whiff_punish_prob
            and not self.override_active
            and fight_state.distance_x < 2.5
            and (
                fight_state.is_opponent_damage
                or fight_state.is_opponent_guard_break
                or fight_state.is_opponent_special_attack
            )
        ):
            attack_queue.clear()
    ...
    def far_approach(self, dash: bool, is_facing_right: bool) -> list[ActionBits]:
        ...
        if dash:
            return [
                ...
                *ActionSequences.back_input(
                    is_facing_right, steps=10 // self.frame_skip
                ),
            ] * 2

        return [
            *ActionSequences.forward_input(
                is_facing_right=is_facing_right, steps=16 // self.frame_skip
            ),
            *ActionSequences.back_input(
                is_facing_right=is_facing_right, steps=12 // self.frame_skip
            ),
            *ActionSequences.forward_input(
                is_facing_right=is_facing_right, steps=8 // self.frame_skip
            ),
            *ActionSequences.back_input(
                is_facing_right=is_facing_right, steps=4 // self.frame_skip
            ),
        ]
    ...
    def mid_approach(self, dash: bool, is_facing_right: bool) -> list[ActionBits]:
        ...
        if dash:
            return [
                *ActionSequences.forward_dash(is_facing_right),
                *ActionSequences.back_input(
                    is_facing_right, steps=8 // self.frame_skip
                ),
            ]
    ...
    def fallback_movement(self, dash: bool, is_facing_right: bool) -> list[ActionBits]:
        ...
        if dash:
            return [
                ...
                *ActionSequences.back_input(
                    is_facing_right, steps=16 // self.frame_skip
                ),
            ]

        return [
            *ActionSequences.back_input(
                is_facing_right=is_facing_right, steps=24 // self.frame_skip
            ),
        ]
    ...
    def _select_attack(self, fight_state: FightState) -> list[ActionBits]:
        ...
        if (
            ...
            # Mirrors the original Footsies BattleAI exactly: with randint(0, 4)
            # the delayed-special branch below is unreachable, matching the
            # upstream behavior where it never triggers.
